...
*/
    ...
    : Graph[(String, Double, Double, (Double, Double, Double, Double)), Double] = {
      ...
      val repV: VertexRDD[(Double, Double)] = g.aggregateMessages[(Double, Double)](
        ...
        mergeMsg = {(m1, m2) => {
            ...
            array.foreach(item=>{a = add(a,repulsionForce( ((item._2._2,item._2._3),m2) ))})
      ...
      = g.vertices.innerJoin(disp)( (_, a, b) => {
          if(b._1.equals(Double.NaN)||b._2.equals(Double.NaN)){preUpdatePos(a,(ran,ran))}
          else{ preUpdatePos(a,b)}
        })
    ...
    : Graph[ (String, Double, Double, (Double,Double,Double,Double)), Double ] = {
      ...
      for(iteration <- 1 to iterations) {
        ...
        val gRep = calcRepulsion( sim, gs )   //计算斥力
        gRep.cache().checkpoint()
        val gBB = calcAttraction(gRep)   //计算引力
        gBB.cache().checkpoint()
        val gAttr = gravity(gBB)   //计算向心力
        gAttr.cache().checkpoint()
        val vNewPositions = gAttr.vertices.mapValues((_, v) => {
    ...
    :Unit={
      ...
      g.checkpoint()
      // 计算图的度分布，并排序
      val maxd = g.degrees.map( x => { (x._2,1) }).reduceByKey( (a,b) => a+b ).sortBy(x => x._1)
      // 度划分比例
      val dbi:Double = 0.2d
      val ddgree = maxd.take((maxd.count() * dbi ).toInt)((maxd.count()*dbi).toInt-1)._2
      // degree打标记，0.0 为false，意思是没有被抽到，3.0 是度筛选节点
      val gbd = g.vertices.innerJoin(g.degrees)((_,a,b) => {
          if(b >= ddgree) {(a._1, a._2, a._3, (a._4._1, a._4._2, b.toDouble, 3.0)) }
          else{(a._1, a._2, a._3, (a._4._1, a._4._2, b.toDouble, 0.0))}
        })
      // 会过滤掉 degree == 0 ，返回类原图
      val ddGraphS = Graph(gbd, g.edges, defaultNode)
      // 一阶图（全部第一次布局 + 抽样部分第一次布局 ）在后面获得
      // 剩余二阶图（剩余的第二次布局）返回子图
      val subGraph_other = ddGraphS.subgraph(vpred = {(_,v) => v._4._4 < ddgree})
      val simpleV = subGraph_other.vertices.innerJoin(
        subGraph_other.vertices.sample( withReplacement = false, sbi ))(
          (_,a,_) => {(a._1, a._2, a._3, (a._4._1, a._4._2, a._4._3, 1.0))})
      //ddGraphS 已经打完标记，标记了在 subGraph_2中 上次sample时采到的点,返回类原图
      val g2_withall = ddGraphS.joinVertices(simpleV)((_,_,b) => b)
      // 计算第一次的图，[ 度 >= degree ] and [ 标记为 1.0 ]
      val g_first = g2_withall.subgraph(vpred = {(_,v) => v._4._3 >= ddgree || v._4._4 == 1.0})
      //两次布局
      val iter = (iterations * 0.5).toInt
      //第一次布局 过滤的度20% + 剩余的20%
      val layout1 = layoutFDFR2( g_first, iter , diet = true) //true 为第一次布局
      // 用第一次布局后的坐标覆盖 原图中节点 得到 g_second
      val g_second = g2_withall.joinVertices(layout1.vertices)( (_,_,b) => b)
